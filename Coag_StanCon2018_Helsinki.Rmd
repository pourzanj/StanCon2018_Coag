...
$$
with $k = 64.0$ and $K = 100.0$ to translate $Fn$ to clot thickness. Clot thickness is plotted below for ODE we simulated above:
...
defines are spline coefficients:
...
$$
Note that $R$, the time that the clot hits 2.0 mm is dependent on the initial value of the system, as well as the parameter values of the system. Taking the partial derivative of the both sides of the equation with respect to $\theta$ yields
...
passing the function $C(t)$ to the algebraic solver available in Stan which uses a modified version of Newton Iterations to find
...
our model on the following trauma patient who we have TEG values for and initial protein values for all the proteins in our model
except for $tPA$ and $PAI$. Our goal here will be to infer this patients initial concentration of $tPA$ given their
TEG measurements. In theory this should be possible because $tPA$ is a protein primarily responsible for clot break up
